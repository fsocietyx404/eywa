...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (!key) {
      await message.channel.send("Usage: .suspend_key XXXX-XXXX-XXXX-XXXX");
      return;
    }
    ...
    await logger(client, "info", "keySuspended", { key, by: message.author.id });

    await message.channel.send(`Key suspended: ${key}`);
  }
};