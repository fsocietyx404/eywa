...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (!key) {
      await message.channel.send("Usage: .revoke_key XXXX-XXXX-XXXX-XXXX");
      return;
    }
    ...
    await logger(client, "info", "keyRevoked", { key, by: message.author.id });

    await message.channel.send(`Key revoked: ${key}`);
  }
};